      ...
      <View style={styles.modalOverlay}>
        <View style={styles.genericModalContainer}>
          ...
          {isLoading ? (
            ...
            <FlatList
              data={safeData}
              // SAFE KEY: Falls back to index if key is undefined
              ...
              ListEmptyComponent={<Text style={{textAlign:'center', margin:20}}>No options available</Text>}
            />
          )}
...
const BookingScreen = () => {
  ...
  useEffect(() => {
    const loadFromPorts = async () => {
      try {
        ...
        // Force array
        setFromPorts(Array.isArray(data) ? data : [data]);
      } catch (error) {
    ...
    const loadAllDegrees = async () => {
      try{
        ...
        // Handle variations (Array vs Object.data vs Object)
        let formattedData = [];
        if(Array.isArray(data)) formattedData = data;
        else if(data?.data && Array.isArray(data.data)) formattedData = data.data;
        else if(data) formattedData = [data];

        setDegrees(formattedData);
      } catch (error) {
  ...
  // --- UPDATED SEARCH FUNCTION WITH STATIC FALLBACK ---
  const handleSearch = async () => {
    // 1. Validate Inputs
    if (!fromPort || !toPort || !travelDate || !selectedClass) {
      Alert.alert(t('booking.missingInfo'), t('booking.missingInfoMessage'));
      return; 
    }
    ...
    try {
      ...
      try {
        ...
        // 3. FALLBACK TO STATIC DATA IF API FAILS (404/500)
        console.log("API Error (404/Network). Using Static Mock Data for Demo.");
        
        // Generate mock dates based on selected Travel Date
        const mockStartDate1 = new Date(travelDate); 
        mockStartDate1.setHours(8, 0, 0, 0); // 8:00 AM
        const mockEndDate1 = new Date(mockStartDate1); 
        mockEndDate1.setHours(11, 30, 0, 0); // 3.5 hrs later

        const mockStartDate2 = new Date(travelDate); 
        mockStartDate2.setHours(14, 0, 0, 0); // 2:00 PM
        const mockEndDate2 = new Date(mockStartDate2); 
        mockEndDate2.setHours(17, 30, 0, 0); // 3.5 hrs later

        tripsData = [
          {
            tripSerial: "101",
            tripName: "Morning Voyage",
            vesselEnglishName: "Bassam Ferry I",
            vesselArabName: "عبارة بسام ١",
            tripStartDate: mockStartDate1.toISOString(),
            tripEndDate: mockEndDate1.toISOString(),
          },
          {
            tripSerial: "102",
            tripName: "Afternoon Voyage",
            vesselEnglishName: "Bassam Ferry II",
            vesselArabName: "عبارة بسام ٢",
            tripStartDate: mockStartDate2.toISOString(),
            tripEndDate: mockEndDate2.toISOString(),
          }
        ];
      }
  
      // 4. Prepare Params & Navigate
      const params = {
        lineCode: toPort?.lineCode || '0',
        ...
        tripsData: JSON.stringify(tripsData), // Pass the real or mock data
      };
